...
import { CalendarDays, Clock, User, Phone, Mail, Calendar, Activity, CheckCircle, AlertCircle, XCircle } from "lucide-react";
...
export default function PatientPage() {
  ...
  const appointmentStats = {
    ...
    pending: appointments.data?.filter((apt: any) => apt.status === 'pending')?.length || 0,
    approved: appointments.data?.filter((apt: any) => apt.status === 'approved')?.length || 0,
    confirmed: appointments.data?.filter((apt: any) => apt.status === 'confirmed')?.length || 0,
    completed: appointments.data?.filter((apt: any) => apt.status === 'completed')?.length || 0,
    cancelled: appointments.data?.filter((apt: any) => apt.status === 'cancelled')?.length || 0,
  };
    ...
    <div className="dashboard-page space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="font-medium">{patient.data?.first_name} {patient.data?.last_name}</p>
            </div>
          </div>
          <div className="flex items-center space-x-3">
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="font-medium">{patient.data?.dob ? formatDate(patient.data.dob) : 'Not set'}</p>
            </div>
          </div>
          <div className="flex items-center space-x-3">
      ...
      <div className="bg-white rounded-lg shadow border p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="text-sm text-gray-500">{recentAppointments.length} of {appointmentStats.total}</span>
        </div>
                  ...
                  <div className="flex-1">
                    <div className="flex items-center space-x-2 mb-2">
                      ...
                      {appointment.priority_rank && (
                        <span className={`text-xs ${getPriorityColor(appointment.priority_rank)}`}>
                          Priority: {appointment.priority_rank}/10
                        </span>
                      )}
                    </div>
                    <h3 className="font-medium text-gray-900 mb-1">Medical Consultation</h3>
                    <p className="text-sm text-gray-600 mb-2">{appointment.medical_description}</p>
                    <div className="flex items-center space-x-4 text-xs text-gray-500">
                      <span>Created: {formatDate(appointment.created_at)}</span>
                      <span>Visiting Status: {appointment.visiting_status}</span>
...
function QuickActionCard({ title, description, href, color }: { title: string; description: string; href: string; color: string }) {
  ...
  return (
    <a href={href} className={`block p-4 border rounded-lg transition-colors ${colorClasses[color as keyof typeof colorClasses]}`}>
